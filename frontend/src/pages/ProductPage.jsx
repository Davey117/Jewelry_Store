...
export default function ProductPage() {
  ...
  const [activeImage, setActiveImage] = useState(null); // Tracks the big image
  ...
  useEffect(() => {
    const loadProduct = async () => {
      try {
        ...
        setProduct(data);
        setActiveImage(data.main_image_url); // Set default image
      } catch (error) {
  ...
  // Combine main image and additional images into one gallery array
  const galleryImages = [product.main_image_url, ...(product.additional_images || [])].filter(Boolean);
    ...
    <div className="max-w-7xl mx-auto px-8 py-16 md:py-24">
      ...
      <div className="flex flex-col md:flex-row gap-16">
        ...
        <div className="w-full md:w-1/2 flex flex-col-reverse md:flex-row gap-4">
            ...
            <div className="flex md:flex-col gap-4 overflow-x-auto md:overflow-visible">
              {galleryImages.map((img, index) => (
                <button 
                  ...
                  className={`w-20 h-24 flex-shrink-0 border transition-all ${activeImage === img ? 'border-amber-600 opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}
                >
          ...
          {/* Main Active Image */}
          <div className="flex-1 bg-gray-50 border border-gray-100 aspect-[4/5] relative">
            {activeImage ? (
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center">
          ...
          <h1 className="text-4xl md:text-5xl font-serif text-gray-900 mb-6 leading-tight">{product.name}</h1>
